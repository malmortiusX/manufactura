"use client";
// src/app/dashboard/export-produccion/produccion-spp/[id]/page.tsx
// Proceso Sin Producto en Proceso — una sola OPG (clase_op=002):
//   OPG1: OP con productos filtrados → consulta componentes ERP → SPG → EPG
...
export default function ProduccionSppDetailPage() {
  ...
  const transmitir = useCallback(async (esReintento = false) => {
    ...
    try {
      ...
      const res  = await fetch(`/api/export-produccion/${id}/transmit-produccion-spp`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({
          bache,
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-slate-800">
            {filtro ? `Sin Prod. en Proceso — ${filtro.nombre}` : "Sin Prod. en Proceso"}
          </h1>
          {filtro && (
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {tr && tr.opg1Num > 0 && (
            <div className="flex items-center gap-1.5 bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs font-semibold px-3 py-1.5 rounded-xl">
              OPG #{tr.opg1Num}
            </div>
          )}
      ...
      {/* Resultado transmisión */}
      {tr && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 px-1">
            <div className="h-px flex-1 bg-indigo-200" />
            <span className="text-xs font-semibold text-indigo-600 uppercase tracking-wider px-2">
              OPG — Producto terminado
            </span>
            <div className="h-px flex-1 bg-indigo-200" />
          </div>
          <OPGPanel
            titulo="OPG — Productos filtrados"
            num={tr.opg1Num}
            result={tr.opg1}
      ...
      {!loading && !error && (
        <>
          ...
          <div className="space-y-3">
            ...
            <XmlBlock title="XML 1 — OPG Orden de Producción"  content={xml1} />
            <XmlBlock title="XML 2 — SPG Consumo"              content={tr?.xmls.xml2 || "// Se genera al transmitir"} />
            <XmlBlock title="XML 3 — EPG Entrega"              content={tr?.xmls.xml3 || "// Se genera al transmitir"} />
          </div>
        </>
      )}
