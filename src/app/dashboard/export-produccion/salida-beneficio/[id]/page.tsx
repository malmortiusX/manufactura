"use client";
// src/app/dashboard/export-produccion/salida-beneficio/[id]/page.tsx
// Igual que Salida Desprese pero el producto en proceso es PP00002
  ...
  const productos = Array.from(
    new Map(
      rows
        ...
        .map((r) => [`${r.CODIGO_PRODUCTO}|${r.LOTE_PRODUCTO}`,
...
function buildXML1(filtro: Filtro, rows: ProductRow[], consecOpg: number): string {
  ...
  const header =
    ...
    pA(filtro.centroOperacion,    3) + pA("OPG", 3) + pN(consecOpg, 8) +
    pA(fecha, 8) + pN(1, 1) + pN(0, 1) + pN(701, 3) +
    pA(filtro.terceroPlanificador, 15) + pA("OPG", 3) + pN(1, 8) +
    pA(filtro.instalacion, 3) + pA("002", 3) +
    pA("", 30) + pA("", 30) + pA("", 30) +
    pA(filtro.nombre, 2000) + pA("", 3) + pA("", 3) + pN(0, 8);
  ...
  const productLines = rows.map((row, i) => {
    ...
    return (
      ...
      pA(filtro.centroOperacion, 3) + pA("OPG", 3) + pN(consecOpg, 8) +
      pN(i + 1, 10) + pN(0, 7) +
      pA(row.CODIGO_PRODUCTO, 50) + pA("", 20) + pA("", 20) + pA("", 20) +
      pA(row.UNIDAD_PRODUCTO, 4) + pN(100, 8) +
      pQ(Number(row.KIL), 15, 4) +
      pA(fecha, 8) + pA(fecha, 8) + pA("0001", 4) +
      bodega + pA("", 4) + pA(row.LOTE_PRODUCTO, 15) + pA("", 2000) + bodega
    );
  });
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
        ...
        <button onClick={handleCopy} className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-slate-600 hover:bg-slate-100 px-2.5 py-1.5 rounded-lg transition-colors">
          {copied
            ? <><svg className="w-3.5 h-3.5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg><span className="text-emerald-500">Copiado</span></>
...
function EstadoBadge({ estado }: { estado: EstadoDoc }) {
  if (estado === "ENVIADO")
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Enviado</span>;
  if (estado === "ERROR")
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-red-50 text-red-700 border border-red-200"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg>Error</span>;
  return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-500 border border-slate-200"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Pendiente</span>;
}

// ── Panel de resultado por documento ──────────────────────────────────────
function DocResultPanel({ label, result, onReintentar, retrying }: {
  label: string; result: DocResult; onReintentar?: () => void; retrying?: boolean;
}) {
  return (
    <div className={`rounded-xl border p-4 space-y-3 ${result.estado === "ENVIADO" ? "border-emerald-200 bg-emerald-50" : result.estado === "ERROR" ? "border-red-200 bg-red-50" : "border-slate-200 bg-slate-50"}`}>
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-semibold text-slate-700">{label}</span>
        <EstadoBadge estado={result.estado} />
      </div>
      {result.exitoso && <p className="text-xs text-emerald-700 flex items-center gap-1.5"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Documento creado exitosamente en ERP</p>}
          ...
          {result.errores.map((e, i) => (
            <div key={i} className="rounded-lg bg-white border border-red-100 px-3 py-2.5 space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {e.valor?.trim() && <span className="font-mono text-xs bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded">Valor: {e.valor.trim()}</span>}
              </div>
      ...
      {result.estado === "PENDIENTE" && !result.exitoso && result.errores.length === 0 && (
        <p className="text-xs text-slate-500">No se envió — depende de que el documento anterior sea exitoso.</p>
      )}
      {result.estado === "ERROR" && onReintentar && (
        <button onClick={onReintentar} disabled={retrying} className="flex items-center gap-1.5 text-xs font-semibold text-white bg-red-600 hover:bg-red-700 disabled:bg-red-300 px-3 py-1.5 rounded-lg transition-colors">
          <svg className={`w-3.5 h-3.5 ${retrying ? "animate-spin" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
...
function LotesResultPanel({ result }: { result: LoteCreacionResult }) {
  ...
  return (
    <div className={`rounded-xl border p-4 space-y-3 ${hayErrores ? "border-amber-200 bg-amber-50" : todoOmitidos ? "border-slate-200 bg-slate-50" : "border-emerald-200 bg-emerald-50"}`}>
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          <svg className={`w-4 h-4 ${hayErrores ? "text-amber-500" : "text-emerald-500"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
          <span className="text-xs font-semibold text-slate-700">Registro de Lotes</span>
        </div>
        <div className="flex items-center gap-1.5 flex-wrap">
          {result.omitidos.length > 0 && <span className="text-xs bg-slate-100 text-slate-600 border border-slate-200 px-2 py-0.5 rounded-full font-medium">{result.omitidos.length} ya existían</span>}
          {result.nuevos.length > 0 && <span className={`text-xs px-2 py-0.5 rounded-full font-medium border ${hayErrores ? "bg-amber-100 text-amber-700 border-amber-200" : "bg-emerald-100 text-emerald-700 border-emerald-200"}`}>{result.creados.length} / {result.nuevos.length} creados</span>}
        </div>
      </div>
      {todoOmitidos && <p className="text-xs text-slate-500 flex items-center gap-1.5"><svg className="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Todos los lotes ya estaban registrados — se omitió el envío al ERP.</p>}
      {result.exitoso && result.nuevos.length > 0 && <p className="text-xs text-emerald-700 flex items-center gap-1.5"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Lotes creados exitosamente en ERP.</p>}
      {hayErrores && result.errores.map((e, i) => (
        <div key={i} className="rounded-lg bg-white border border-amber-100 px-3 py-2.5 space-y-1">
          <span className="font-mono text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">Línea {e.nroLinea}</span>
...
export default function SalidaBeneficioDetailPage() {
  ...
  const transmitir = useCallback(async (esReintento = false) => {
    ...
    else { setTransmitting(true); setTransmitResult(null); setLotesResult(null); }
    setTransmitError(null);
    ...
    try {
      ...
      if (uniqueLotes.length > 0) {
        ...
        if (!lotesData.exitoso && lotesData.nuevos.length > 0) {
          setTransmitting(false); setRetrying(false); return;
        }
      }

      // Paso 2 — Consecutivo OPG
      ...
      rows.forEach((r) => {
        if (r.CODIGO_PRODUCTO.trim()) lotesPorProducto[r.CODIGO_PRODUCTO.trim()] = r.LOTE_PRODUCTO?.trim() ?? "";
      });
      ...
      setTransmitting(false); setRetrying(false);
    }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => router.back()} className="p-2 rounded-lg hover:bg-white border border-transparent hover:border-slate-200 text-slate-400 hover:text-slate-600 transition-all">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        </button>
        <div>
          <h1 className="text-lg font-semibold text-slate-800">
            {filtro ? `Salida Beneficio — ${filtro.nombre}` : "Salida Beneficio"}
          </h1>
          {filtro && (
        ...
        <div className="ml-auto flex items-center gap-3">
          {bache !== null && (
            <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs font-semibold px-3 py-1.5 rounded-xl">
              <svg className="w-3.5 h-3.5 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg>
              Lote {bache}
            </div>
          )}
          {consecOpg > 0 && (
          ...
          <button onClick={cargarDatos} disabled={loading || transmitting} className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-slate-700 hover:bg-white border border-transparent hover:border-slate-200 px-3 py-2 rounded-xl transition-all disabled:opacity-40">
            <svg className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
            Actualizar
          </button>
          {!loading && rows.length > 0 && (
            <button onClick={() => transmitir(false)} disabled={transmitting || loading} className="flex items-center gap-2 text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-300 text-white px-4 py-2 rounded-xl transition-all shadow-sm">
              {transmitting
                ? <><svg className="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>Transmitiendo…</>
                : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>Transmitir al ERP</>
              }
            </button>
          )}
        </div>
      ...
      {transmitResult && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-slate-700">{lotesResult ? "Paso 2 — " : ""}Resultado de transmisión</span>
              <span className="text-xs font-semibold bg-indigo-50 text-indigo-600 border border-indigo-200 px-2 py-0.5 rounded-full">OPG #{transmitResult.numeroOpg}</span>
            </div>
            {transmitResult.orden.exitoso && transmitResult.consumo.exitoso && transmitResult.entrega.exitoso
              ? <span className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Todos los documentos enviados</span>
              : <span className="flex items-center gap-1.5 text-xs font-semibold text-red-600"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Hay errores — revisa los detalles</span>
            }
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <DocResultPanel label="Orden de Producción"  result={transmitResult.orden}   onReintentar={!transmitResult.orden.exitoso   ? () => transmitir(true) : undefined} retrying={retrying} />
            <DocResultPanel label="Consumo de Producción" result={transmitResult.consumo} onReintentar={!transmitResult.consumo.exitoso && transmitResult.orden.exitoso   ? () => transmitir(true) : undefined} retrying={retrying} />
            <DocResultPanel label="Entrega de Producción" result={transmitResult.entrega} onReintentar={!transmitResult.entrega.exitoso && transmitResult.orden.exitoso && transmitResult.consumo.exitoso ? () => transmitir(true) : undefined} retrying={retrying} />
          </div>
        </div>
      )}
          ...
          <div className="overflow-x-auto">
                ...
                {loading
                  ? Array.from({ length: 5 }).map((_, i) => (
                      <tr key={i} className="animate-pulse">
                        {Array.from({ length: 9 }).map((__, j) => <td key={j} className="px-5 py-4"><div className="h-4 bg-slate-100 rounded w-3/4" /></td>)}
                      </tr>
                    ))
                  : rows.map((row, i) => (
                      <tr key={i} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-5 py-3"><span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-600">{row.UNIDAD_PRODUCTO}</span></td>
            ...
            {!loading && rows.length === 0 && (
              <div className="text-center py-12 text-slate-400">
                <svg className="w-10 h-10 mx-auto mb-3 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
